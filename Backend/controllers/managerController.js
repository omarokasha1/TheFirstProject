...
const managerCtrl = {
  ...
  getPromotRequest: async (req, res, next) => {
    ...
    try {
      ...
      const id = user.id
      console.log(id)
      promotRequest = await RequestToManager.find().populate('authorPromoted', '-__v -password').select('-__v -userEnrolled -trackPublished -coursePublished')
      if (!promotRequest) {
  ...
  getEnrollRequest: async (req, res, next) => {
    ...
    try {
      ...
      const id = user.id
      console.log(id)
      enrollRequests = await enrollRequest.find().populate('courseId userId', '-__v -password').select(' -__v ')
      if (!enrollRequests) {
  ...
  getCourseRequest: async (req, res, next) => {
    ...
    try {
      ...
      const id = user.id
      console.log(id)
      courseRequests = await courseRequest.find().populate('authorId courseId', '-__v -password').select(' -__v ')
      if (!courseRequests) {
  ...
  getTarckRequest: async (req, res, next) => {
    ...
    try {
      ...
      const id = user.id
      console.log(id)
      trackRequests = await trackRequest.find().populate('authorId trackId', '-__v -password').select(' -__v ')
      if (!trackRequests) {
  ...
  getAllUsers: async (req, res) => {
    try {
      const users = await User.find({ isAuthor: false, isManager: false, isAdmin: false }).populate('wishList myCourses myTracks').select('-__v -password')
  ...
  getAllAuthors: async (req, res) => {
    try {
      const users = await User.find({ isAuthor: true, isManager: false, isAdmin: false }).populate('wishList myCourses myTracks').select('-__v -password')
