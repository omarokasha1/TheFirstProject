...
const userCtrl = {
  ...
  getUserId: async (req, res) => {
    try {
      ...
      const user = await Users.findById(req.params.id).populate('myCourses wishList myTracks','-__v -password').select('-__v -password')
  ...
  changePassword: async (req, res) => {
    ...
    const validateError = Users.validateUserPassword(req.body)
    ...
    try {
      ...
      //* if password doesnt match return to user an error message 
      if (!checkPassword) {
        return res.status(200).json({ status: 'false', message: 'old password incorrect!' })
      }
  ...
  getWishCourses: async (req, res) => {
    ...
    try {
      ...
      console.log(course)
      let cour = await Course.find({ _id: course.wishList }).populate('author contents','-__v -password')
      res.json({ status: 'ok', message: ' Getting WishList Success', wishList:cour})
    } catch (error) {
      console.log(error)
      return res.status(500).json({ status: 'false', message: error.message })
    }
  ...
  getWishTracks: async (req, res) => {
    ...
    try {
      ...
      let course = await Users.findById(ObjectId(id)).populate('myTracks')
  ...
  getEnrollCourse: async (req, res) => {
    ...
    try {
      ...
      let course = await Users.findOne({ _id: ObjectId(id) })
      console.log('--------------------------------------------')
      console.log(course.myCourses)
      console.log('--------------------------------------------')

      let cour = await Course.find({ _id: course.myCourses }).populate('author contents learner wishers','-__v -password').select('-__v -password')
      console.log(cour)
      /*  let x = await Users.findOne({id:course.myCourses.author}).populate('author')
       console.log(x) */
      res.json({ status: 'ok', message: ' Enroll Success', myCourses: cour })
    } catch (error) {
      console.log(error)
      return res.status(500).json({ status: 'false', message: error.message })
    }
  ...
  getEnrollTrack: async (req, res) => {
    ...
    try {
      ...
      let track = await Users.findOne({ _id: ObjectId(id) })
      console.log(track.myTracks)
      let trac = await Track.find({ _id: track.myTracks }).populate('author learner courses','-__v -password').select('-__v -password')
      res.json({ status: 'ok', message: ' Enroll Success', myTracks: trac })
    } catch (error) {
      console.log(error)
      return res.status(500).json({ status: 'false', message: error.message })
    }
