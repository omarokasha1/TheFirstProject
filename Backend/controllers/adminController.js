...
const managerCtrl = {
  ...
  getAllUsers: async (req, res) => {
    try {
      const users = await Users.find({ isAuthor: false, isManager: false, isAdmin: false }).populate('wishList myCourses myTracks').select('-__v -password')
  ...
  getAllManagers: async (req, res) => {
    try {
      const users = await Users.find({ isManager: true, isAdmin: false }).populate('wishList myCourses myTracks').select('-__v -password')
  ...
  getAllAuthors: async (req, res) => {
    try {
      const users = await Users.find({ isAuthor: true,isManager:false, isAdmin: false }).populate('wishList myCourses myTracks').select('-__v -password')
